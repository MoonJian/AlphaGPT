...
class DataEngine:
    ...
    def load(self):
        ...
        def robust_norm_rolling(x_array, window=300):
            """
            使用滚动窗口进行 Robust Normalization，杜绝未来函数。
            window: 窗口大小，通常取较大的周期，如 300 或 500，以保证统计稳定性。
            """
            series = pd.Series(x_array)
            
            # 1. 计算滚动中位数 (Rolling Median)
            # min_periods=window 表示直到攒够了 window 个数据才开始计算，避免前期波动过大
            roll_median = series.rolling(window=window, min_periods=window).median()
            
            # 2. 计算滚动 MAD (Rolling Median Absolute Deviation)
            # 注意：在 Pandas 中直接计算滚动 MAD 比较慢且复杂，
            # 这里我们先算出偏差，再算偏差的滚动中位数。
            # 这是一个近似处理，严格的滚动MAD计算量极大。
            abs_diff = (series - roll_median).abs()
            roll_mad = abs_diff.rolling(window=window, min_periods=window).median() + 1e-6
            
            # 3. 计算 Z-score 变体
            res = (series - roll_median) / roll_mad
            
            # 4. 填充前面的 NaN (因为 rolling 前 window 个数据是空的)
            res = res.fillna(0) 
            
            return np.clip(res.values, -5, 5).astype(np.float32)

        # 构建特征张量
        self.feat_data = torch.stack([
            torch.from_numpy(robust_norm_rolling(ret, window=960)).to(DEVICE),
            torch.from_numpy(robust_norm_rolling(ret5, window=960)).to(DEVICE),
            torch.from_numpy(robust_norm_rolling(vol_chg, window=960)).to(DEVICE),
            torch.from_numpy(robust_norm_rolling(v_ret, window=960)).to(DEVICE),
            torch.from_numpy(robust_norm_rolling(trend, window=960)).to(DEVICE)
        ])
...
class DeepQuantMiner:
    def __init__(self, engine):
        ...
        self.opt = torch.optim.AdamW(self.model.parameters(), lr=1e-5, weight_decay=1e-5) # AdamW 防止过拟合
...
def final_reality_check(miner, engine):
    ...
    plt.plot(list(range(len(equity))), equity, label='Strategy (Open-to-Open)', linewidth=1.5)
    ...
    plt.plot(list(range(len(bench_equity))), bench_equity, label='Benchmark (CSI 300)', alpha=0.5, linewidth=1)
